...
class RenderSceneNode (sn.SceneBaseNode, sn.SceneBaseProps) : 
    
    bl_idname = "RenderSceneNode"
    bl_label = "Render Settings"
    
    sceneType = "Render Settings"
    sceneTypeIcon = "SETTINGS"
    
    seq : bpy.props.IntProperty(name="Sequence", description="The number of the sequence of this scene", min=0)
    
    def init(self, context) : 
        super().init(context)
        pnt.PipelineNodeTree.get().renderSettingsNodes.append(self)
...
def register () : 
    bpy.utils.register_class (RenderSceneNode)

def unregister () : 
    bpy.utils.unregister_class (RenderSceneNode)
