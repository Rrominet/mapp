...
class AnimSceneNode (sn.SceneBaseNode, sn.SceneBaseProps) : 
    bl_idname = "AnimSceneNode"
    bl_label = "Animation"
    
    sceneType = "Animation"
    sceneTypeIcon = "MOD_TIME"
    
    seq : bpy.props.IntProperty(name="Sequence", description="The number of the sequence of this scene", min=0)
    
    def init(self, context) : 
        super().init(context)
        pnt.PipelineNodeTree.get().animationNodes.append(self)
...
def register () : 
    bpy.utils.register_class (AnimSceneNode)

def unregister () : 
    bpy.utils.unregister_class (AnimSceneNode)
