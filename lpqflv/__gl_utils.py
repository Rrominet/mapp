...
class Font :
    ...
    def __init__(self, id=0, pos=(0,0,0), size=(0,0), color= (1,1,1,1)) : 
        self.id = id
        self.pos = pos
        self.size = size
        self.color = color
...
class Drawer : 
    ...
    def __init__(self, drawFunc, args=(), space='VIEW_3D', type="POST_PIXEL") : 
    ...
    def addHandler(self) :
        ...
        elif self.space == 'EMPTY' : 
            return 

        elif self.space == "VIEW_3D" : 
            self.handler = bpy.types.SpaceView3D.draw_handler_add(self.drawFunc, 
                                                                      self.args, 
                                                                      self.region_type,
                                                                      self.type)
        
        elif self.space == 'NODE_EDITOR': 
            self.handler = bpy.types.SpaceNodeEditor.draw_handler_add(self.drawFunc, 
                                                                      self.args, 
                                                                      self.region_type,
                                                                      self.type)
    def removeHandler(self) : 
        ...
        if self.space == 'NODE_EDITOR' :
            bpy.types.SpaceNodeEditor.draw_handler_remove(self.handler, self.region_type) 
            self.handler = None
...
def drawText(font) : 
    blf.position (font.id, font.pos[0], font.pos[1], font.pos[2])
    blf.size(font.id, font.size[0], font.size[1]) 
    blf.color(font.id, font.color[0], font.color[1], font.color[2], font.color[3])
    blf.draw(font.id, font.text)
        
def draw3DImage(path, space="VIEW_3D") : 
    pass

class ImageBase : 
    def __init__(self, path, coord=[5, 5], space="VIEW_3D") : 
        self._path = path
        self._coord = coord
        self._space = space
        self._w = 0;
...
class _3DImage(ImageBase) : 
    def __init__(self, path, coord=[0, 0, 0], space="VIEW_3D") : 
